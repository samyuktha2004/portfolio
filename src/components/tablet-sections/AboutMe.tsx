...
export function AboutMe() {
  return (
    <div className="space-y-6 tablet-section">
      ...
      {/* TL;DR Section */}
      <div className="bg-gradient-to-br from-[#87CEEB] to-[#6BA5D6] rounded-2xl p-4 sm:p-6 border-2 border-white/20 shadow-lg">
        <h4 className="text-white mb-3">✨ Quick Summary</h4>
        <p className="text-white/90 mb-3 text-sm sm:text-base">{aboutMe.tldr.role}</p>
        <div className="space-y-2">
          {aboutMe.tldr.highlights.map((highlight, i) => (
            <div key={i} className="flex items-start gap-2 text-white/90 text-xs sm:text-sm">
      ...
      <div className="bg-[#FFF0F5] rounded-2xl p-4 sm:p-6 border-2 border-[#FFB6C1]/30 shadow-lg">
        <h4 className="text-[#fd6698] mb-4">🏆 Competition Highlights</h4>
        <div className="space-y-3 text-gray-700 text-sm sm:text-base">
          {aboutMe.competitionHighlights.map((highlight, i) => (
            <div key={i} className="flex items-start gap-2">
              <span className="text-[#FFB6C1] flex-shrink-0 mt-1">•</span>
      ...
      <div className="bg-gradient-to-br from-[#87CEEB] to-[#6BA5D6] rounded-2xl p-6 sm:p-8 text-center shadow-xl border-2 border-white/20">
        ...
        <div className="bg-white/10 backdrop-blur-sm rounded-xl p-4 mb-4 border border-white/20">
          <div className="flex items-center justify-center gap-2 mb-2">
            <span className="text-white text-sm sm:text-base">{aboutMe.availability.status}</span>
          </div>
