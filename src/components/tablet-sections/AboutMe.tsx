...
export function AboutMe() {
  return (
    <div className="space-y-6 tablet-section">
      ...
      {/* Fun Facts */}
      <div className="bg-[#FFF0F5] rounded-2xl p-4 sm:p-6 border-2 border-[#FFB6C1]/30 shadow-lg">
        <h4 className="text-[#FFB6C1] mb-4">Fun Facts About Me</h4>
        <div className="space-y-2">
          {[
            '🍵 I collect unique tea cups from around the world',
            '🎨 My favorite color is (surprise!) baby pink',
            '📚 I read at least one design book per month',
            '🎮 I love playing cozy games for creative inspiration',
            '✈️ I love travelling to new places and cultures',
            '🌸 My desk is always decorated with fresh flowers',
          ].map((fact, i) => (
            <div
              key={i}
              className="flex items-center gap-2 text-gray-700 bg-white/50 rounded-lg p-3 text-sm sm:text-base"
            >
      ...
      <div className="bg-gradient-to-br from-[#87CEEB] to-[#6BA5D6] rounded-2xl p-6 sm:p-8 text-center shadow-xl border-2 border-white/20">
        <div className="mb-4">
          ...
          <h4 className="text-white mb-2">Ready to Work Together?</h4>
          <p className="text-white/90 text-sm sm:text-base max-w-md mx-auto">
            I offer UI/UX Design, Frontend Development, and Full-Stack services. Let's create something amazing! ✨
          </p>
        </div>
        <div className="bg-white/10 backdrop-blur-sm rounded-xl p-4 mb-4 border border-white/20">
          <div className="flex items-center justify-center gap-2 mb-2">
            <div className="w-3 h-3 rounded-full bg-[#4ade80] animate-pulse" />
            <span className="text-white text-sm sm:text-base">🟢 Available for Freelance & Internships</span>
          </div>
          <p className="text-white/80 text-xs sm:text-sm">Starting January 2026</p>
        </div>
        <button
          onClick={() => {
